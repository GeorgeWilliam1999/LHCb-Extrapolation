# A1_Stage_count_sweep — how many stages the magnet crossing needs
...
`../../Block_0_first_pass/S1_Simple_first_pass` is not the one to compare against).
...
`S2b_One_step_network_v2`'s 177 – 235 µm, data twin 162 – 204 µm against 162 – 208 µm, on a
...
3. **The pre-stated ceiling numbers (~14 mm at q = 2, ~4.3 mm at q = 4) do not describe
   this population.** They come from `../../Block_0_first_pass/S1_Simple_first_pass`, which measured leg B on 32
...
| [measure_scheme_ceiling.py](measure_scheme_ceiling.py) | solves the exact q-stage scheme on **this experiment's own test states**, so the ceiling is like-for-like with what the networks are scored on. `solve_leg` is imported from `../../Block_0_first_pass/S1_Simple_first_pass/exact_scheme.py`, not copied | `results/scheme_ceiling_same_population_q<qq>.json` |
...
- Training population: the official TestFileDB sample via
  ...
  `S2b_One_step_network_v2` used; 1979 train / 2062 val / 2018 test states after the fiducial
...
it is worth changing. In `S2b_One_step_network_v2` the 150 was the cap on the *stall* phase
