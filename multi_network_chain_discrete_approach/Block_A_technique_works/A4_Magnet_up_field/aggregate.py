...
Three magnet-down comparators, because they are not interchangeable:
  ...
  * `../A2_Network_size_and_seed_study` (A1) at width 50, depth 4 - the closest
  ...
  * `../../Block_0_first_pass/S2b_One_step_network_v2` - the original three-seed baseline, run with four
  ...
  * the exact-scheme ceiling, measured by `ceiling_up.py` on both polarities,
    on both the 32 momentum-stratified legs of `../../Block_0_first_pass/S1_Simple_first_pass` and the
    ...
    `../A1_Stage_count_sweep/results/scheme_ceiling_same_population_q08.json` to the
    last digit, which is the cross-check that the solver copied into
    `ceiling_up.py` is the shared one.

The 29 um figure in `../../Block_0_first_pass/S1_Simple_first_pass/results/scheme_error_vs_q.csv` is not
...
V2 = os.path.join(HERE, "..", "..", "Block_0_first_pass", "S2b_One_step_network_v2", "results", "summary.csv")
...
def main():
    ...
    for mode, label in (("physics", "network, physics loss (label-free)"),
        ...
        if v:
            add(label, "down", [float(r["test_endpoint_med_um"]) for r in v],
                "A1 A2_Network_size_and_seed_study 50x4; converged %d/%d"
        ...
        add(label, "down", [float(r["endpoint_med_um"]) for r in d],
            "S2b_One_step_network_v2 baseline, 3 seeds, 4 BLAS threads")
    ...
    for pol in ("up", "down"):
        out.append({"quantity": "exact-scheme ceiling, 32 stratified legs",
                    ...
                    "note": "the S1_Simple_first_pass population; max_um is the "
