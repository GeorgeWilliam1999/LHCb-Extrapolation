...
This is the experiment of `../../Block_0_first_pass/S2b_One_step_network_v2` — the paper's one-step
...
|---|---|---|
| [build_dataset.py](build_dataset.py) | one call to `_shared.prepare.frozen_leg_dataset(q=8, field='down', n_train=2000, seed=20260718, rebase_mm=60, fiducial=True)`, asserting the split counts against the verified baseline | `results/frozen_leg_q08.npz` (+ `_meta.json`); 1979 / 2062 / 2018 states, as in `../../Block_0_first_pass/S2b_One_step_network_v2` |
...
|---|---:|---|
| exact scheme at q = 8, no network | **23 um** | `../A1_Stage_count_sweep/measure_scheme_ceiling.py` on these same 2018 test states (22.5 um). The 29 um quoted in `../../Block_0_first_pass/S2_One_step_network*` was measured on a different population — 32 momentum-stratified legs — and is not the ceiling for this test set. |
| `../../Block_0_first_pass/S2b_One_step_network_v2`, 4x50, 3 seeds | 183 um | `../../Block_0_first_pass/S2b_One_step_network_v2/results/summary.csv`. Those runs used four BLAS threads; everything here is single-threaded, so they are not bit-comparable — the 4x50 row above (222 um over 10 seeds, range 165-244) is the like-for-like number. |
...
All ten were rerun rather than reused from `../../Block_0_first_pass/S2b_One_step_network_v2`, because
