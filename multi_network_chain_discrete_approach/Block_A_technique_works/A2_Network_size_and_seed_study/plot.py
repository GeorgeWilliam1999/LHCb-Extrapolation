...
aggregate.py) and recomputes nothing.  Writes
  ...
  figures/floor_vs_architecture.png  median test endpoint error per architecture
                                     ...
                                     ceiling, the S2b_One_step_network_v2 4x50
...
# ../A1_Stage_count_sweep/measure_scheme_ceiling.py (22.5 um).  The 29 um quoted
# in ../../Block_0_first_pass/S2_One_step_network* was measured on a different population (32
# momentum-stratified legs) and is not the ceiling for this test set.
CEILING_JSON = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "A1_Stage_count_sweep",
...
# The verified baseline, ../../Block_0_first_pass/S2b_One_step_network_v2/results/summary.csv (physics,
# seeds 0-2, 4x50 network, four BLAS threads).
V2_SUMMARY = os.path.join(HERE, "..", "..", "Block_0_first_pass", "S2b_One_step_network_v2", "results",
                          "summary.csv")
...
def fig_floor(runs, arch):
    ...
    if v2 is not None:
        ...
        ax.text(0.015, v2 * 1.04,
                "S2b_One_step_network_v2, 4x50, 3 seeds, 4 threads: %.0f um" % v2,
