...
PORTED VERBATIM (construction + verification) from the van der Pol study,
Van_Der_Pole/discrete_time_network/irk.py (the clean-start Raissi rebuild);
only this header differs. The construction is problem-independent: nodes and
weights from the Legendre roots, the matrix A from barycentric collocation
integrals, and `verify_tableau` checks every identity including the
literature tableaus for q = 1, 2, 3. The van der Pol `exact_step` is NOT
ported -- the LHCb leg solver lives in exact_scheme.py (non-autonomous ODE,
per-leg step, slope/position scaling).
"""
