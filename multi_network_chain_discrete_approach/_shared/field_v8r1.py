#!/usr/bin/env python3
"""Vendored copy of the canonical LHCb v8r1 field-map loader.

ORIGIN (vendored 2026-09-05, byte-for-byte apart from this header):
    /data/bfys/gscriven/Track_Extrapolation_work_archive/track-extrapolation-pinn/core/field_v8r1.py
    archive repository commit 1faa97e085918082fd8ff15e60aa7932bb588786

Why vendored: the earlier modules reached into the archive with a
`sys.path.insert`, which makes every downstream experiment depend on a
read-only tree outside this repository. The copy is gated in
`vendoring_parity.py`: field values on 200,000 random in-map points must be
bit-identical to the archive import (max |difference| exactly 0), and the md5
of the map file itself is recorded alongside. Do not edit the body below; if
the archive module ever changes, re-vendor and re-run the gate.

The map file is read from CVMFS and is not part of this repository.
"""
