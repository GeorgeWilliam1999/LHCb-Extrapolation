#!/bin/bash
# Block F farm keeper. Every 30 minutes it
...
#   tmux new-window -t claude-phone -n blockF-keeper <this script>
cd "$(dirname "$0")/.."
...
while true; do
  ...
  (cd /data/bfys/gscriven/LHCb_Extrapolation_Project/single_network_chain_discrete_approach/Block_F_reweighted_loss/F2_Analysis && PYTHONNOUSERSITE=1 /data/bfys/gscriven/conda/envs/TE/bin/python prune_active.py --block F --write >> "$OLDPWD/$LOG" 2>&1)
  ...
  echo "$(date '+%F %T') $done_runs of 3 at their cap, $queued in the queue" >> $LOG
