#!/usr/bin/env python
"""F1 - Block E's round trainer with ONE change: the weights inside the loss.

This file is `Block_E_single_network_chain/E1_Network_grid/train_network.py`
with the loss swapped for `F0_Weighting/weighted_loss.py` and nothing else
touched. `results/train_weighted.diff` is the diff against it, and
...
The network itself is imported from Block E, not copied, so it cannot drift.
  ...
  loss      the label-free RK-PINN residual of `_shared.model`, weighted: each
            ...
            10-50 GeV band weighted up. `--weighting` picks the mode (`full`,
            the ablations `no_lever` / `no_track` / `no_window`, or `blockE`
            for Block E's own weights). See `weighted_loss.py` for the form
            and why each factor is there
...
Usage:
    python train_weighted.py --N 64 --q 2
    python train_weighted.py --N 64 --q 2 --weighting blockE --n-train 300 --stop-after 3
    python train_weighted.py --N 2 --q 2 --n-train 300 --n-eval 200 --round-cap 2 --out results/smoke
