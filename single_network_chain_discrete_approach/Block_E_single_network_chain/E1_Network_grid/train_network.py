#!/usr/bin/env python
"""E1 - train ONE Block E network for one (N, q), over the whole crossing, in rounds.

The plan (Notion to-do "Train one network per step length and chain it to
itself across the magnet", decisions settled 2026-09-16):

  loss      the label-free RK-PINN loss of `_shared.model.physics_loss`: every
            one of the q + 1 outputs, stepped back through the implicit
            Runge-Kutta equations of the same step, must give back the input;
            the start plane is per track (the network's extra input)
...
Usage:
    python train_network.py --N 64 --q 8
    python train_network.py --N 2 --q 2 --n-train 300 --n-eval 200 --round-cap 2 --out results/smoke
    python train_network.py --N 256 --q 16 --timing-restarts 1 --out results/timing
...
def main(argv=None):
    ...
    run = os.path.join(a.out, "N%03d_q%02d" % (a.N, a.q))
    ...
    if os.path.exists(paths["scale"]):
        ...
        S1, _, _ = draw_states(rk6_on_planes, z_starts, a.states_per_round, rng1)
    ...
    def loss_now():
        return physics_loss(model, rates, St, dz, zn, A, b, zt)
