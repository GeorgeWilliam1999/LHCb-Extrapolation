...
error is $+0.965$. Trained on the magnet-up polarity, for which no labelled
sample exists anywhere in the project, the same construction reaches
...
\textbf{Correction, 6 September 2026: the sample is magnet-up.}
The conditions tag \texttt{sim-20231017-vc-mu100} is the \textbf{magnet-up}
configuration. This was established while building the magnet-to-magnet data set
of the next block, by propagating each particle's forward cross-magnet leg from
its last upstream-tracker plane and comparing the result with that particle's
\emph{own} first scintillating-fibre state, which the sample supplies for free.
Over all $15{,}257$ selected particles the leg propagated with
\texttt{field.v8r1.up.bin} lands a median $1.73\mm$ from the real state
($14.8\mm$ at $1$--$2$ GeV, $0.28\mm$ at $25$--$200$ GeV), with the sign of the
bend correct for $100.0$ percent of particles; propagated with
\texttt{field.v8r1.down.bin} it lands $883\mm$ away ($3973\mm$ and $169\mm$ in
the same two bands) with the sign wrong for $100$ percent, and $2$ percent of
legs leave the field map altogether. The two maps are exact negatives, so
polarity changes only the sign of the bend.
\smallskip

\textbf{Every label and score in this paper was computed with the magnet-down
map}: the $Y$ column of the training set, the frozen-leg experiments of
Sections~\ref{sec:baseline} to~\ref{sec:size}, and the general-leg and
residual experiments of Sections~\ref{sec:general} and~\ref{sec:residual}. Four
consequences follow, and no more.
\begin{enumerate}\setlength{\itemsep}{1pt}
  \item \textbf{Every method conclusion stands.} Each experiment compared a
  network against the same equations and the same engine: scheme-limited against
  network-limited, the optimisation-limited floor, the residual redesign, and
  chaining are all internal comparisons and are untouched.
  \item \textbf{Any statement that those labels are where the simulated
  particle went is wrong}, and the material-floor gate G2 of
  Section~\ref{sec:gates} must be re-read on the magnet-up map.
  \item \textbf{The magnet-up experiment of Section~\ref{sec:magnetup} is not
  ``a polarity with no labels'' but the sample's true polarity}, and it is the
  only experiment here scored against the physically right field. Its result is
  unchanged --- the same floor, $229$ against $222\um$ --- and its framing is
  not.
  \item The magnet-to-magnet data set, the fine reference and the tables now in
  progress are built on the magnet-up map.
\end{enumerate}
\smallskip

{\raggedright Source:
\texttt{R/Magnet\_tracks\_dataset/}\allowbreak\texttt{results/polarity\_check.json}
and \texttt{R/Magnet\_tracks\_dataset/README.md}.\par}}}
\end{center}
...
\begin{enumerate}
  ...
  magnet-down polarity.\footnote{The sample itself is magnet-up; see the
  correction box in Section~\ref{sec:sample}. The labels are therefore
  field-only propagations of the right start states through the wrong polarity,
  which is a statement about what the labels \emph{are} and not about any
  comparison made with them.} The labels are used to score every experiment and
  to train the data twin; the physics loss never touches them.
...
extrapolator.\footnote{G2 as tabulated was measured against labels computed with
the magnet-down map, so on the cross-magnet leg it is not a material-only
residual and must be re-read on the magnet-up map. See the correction box in
...
\subsection{Training where no labels exist: the magnet-up polarity}
\label{sec:magnetup}
...
ever been made for it in this project.\footnote{Magnet up is in fact the
polarity the sample was simulated with, established after this experiment was
run; see the correction box in Section~\ref{sec:sample}. The experiment is
therefore the only one in this paper scored against the physically right field.
Its numbers are unchanged; ``no labels have been made for it'' remains true, and
``a polarity the sample is not'' would not be.}
...
\begin{table}[tbp]
  ...
  sample has ever been produced for the magnet-up polarity in this project,
  which is nonetheless the sample's own polarity
  (Section~\ref{sec:sample}).
...
splits on magnet down and \emph{none} on magnet up: the
sample's charge and angular distribution is not symmetric in $x$, and magnet
down is the one that bends these particular soft tracks out of the
map while magnet up bends them back in. The same asymmetry appears
...
\begin{table}[tbp]
  ...
  sizes; column 3 is the same leg on the magnet-up polarity; columns 4 and
...
in the project} --- which is, in fact, the sample's own polarity
(Section~\ref{sec:sample}) --- it reaches the same floor, $229$ against
$222\um$, and sits the same distance above that polarity's own ceiling
(Table~\ref{tab:magnetup}).
...
labelled sample exists, and which is the polarity the sample itself was
simulated with (Section~\ref{sec:sample}).
...
seed $20260718$. The conditions tag is \textbf{magnet-up}, while every label in
the set was computed with \texttt{field.v8r1.down.bin}; the test that settles it
is \texttt{R/Magnet\_tracks\_dataset/results/polarity\_check.json}, and the
correction box in Section~\ref{sec:sample} states what it does and does not
affect.
