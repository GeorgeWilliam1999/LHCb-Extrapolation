...
are therefore global minima of the residual term, with basins that grow as the
horizon grows, and only the initial-condition term prefers the true solution.
They measure success rates falling monotonically with horizon and with proximity
of the start to a fixed point, and report that larger networks improve matters
slightly but repair nothing, concluding that the difficulty is bound to
optimisation complexity rather than to expressive power. They propose no remedy.
...
pseudo-time stepping with collocation resampling, which adds a term penalising
change from the previous iterate and thereby amplifies the hidden transition
layer by a factor that grows as the layer sharpens.

Babic, Rohrhofer and Geiger~\cite{babic2025} propose a regulariser that
penalises a candidate for sitting still inside a locally unstable region: a
product of the sum of positive real parts of the Jacobian eigenvalues at the
network's output and a Gaussian in the network's own speed, with a coefficient
that decays to zero halfway through training. It is inert on any moving
candidate by construction. Demonstrated on van der Pol to a horizon of about two
periods, it converts failure into success there.

The van der Pol study measured all three on its own problem. The finding that
matters for LHCb is the ordering: the regulariser turns parking at the fixed
point into a different member of the same family, a flow-following splice, and
only pseudo-time stepping prices the whole family rather than one member of it.
Across $240$ converged runs, pseudo-time stepping was the only intervention that
worked past two periods.
...
\subsection{The equation of motion in $z$}
\label{sec:eom}
...
\subsection{Why the ceiling floors at about $23\um$}
\label{sec:floor}
...
\begin{table}[tbp]
  ...
  \begin{tabular}{clcr}
    ...
    \midrule
    A & vertex fetch: first vertex-detector state back to its collision vertex
      & $341\mm$, backward & $49{,}014$ \\
    B & cross-magnet: last plane before the magnet to first plane after, both
        directions & $5175\mm$ & $41{,}398$ \\
    C & plane-to-plane: consecutive sensor crossings, forward, at most three
        per particle & $70\mm$ & $205{,}887$ \\
    D & downstream to vertex: first post-magnet state back to the collision
        vertex & $-8.6$ m & $27{,}234$ \\
    \midrule
...
\paragraph{The $29\um$ correction.}
...
\begin{figure}[tbp]
  ...
  endpoint error against track momentum, one point per test track per seed
  median, with binned medians overlaid; the two polarities trace the same curve,
  falling from about one millimetre at $2$ GeV to about $100\um$ above $20$ GeV.
  Right: three test tracks propagated from the same start states under both
  polarities, showing the network's eight stage states as markers on top of the
  double-precision reference curves; the trajectories curve in opposite
  directions, which is the check that the label-free network learned the sign of
  the bending and not merely the geometry of the detector.}
  \label{fig:magnetup}
\end{figure}
...
\begin{figure}[tbp]
  ...
  measured on that same population. The two architectures are shown side by side
  within each panel. The middle row shows the network beating the straight line
  by two orders of magnitude on the cross-magnet leg while sitting a factor $56$
  above the ceiling; the top and bottom rows show the opposite situation, where
  the straight line lies below every network point, so on the short legs the
  network is worse than ignoring the magnet.}
...
\begin{table}[tbp]
  ...
  the range in brackets. Every column was re-scored in one pass on the same test
  states by \texttt{aggregate\_residual.py}, which is why the absolute-output
  $4 \times 200$ column differs by up to two percent from
  Table~\ref{tab:general}. Source:
  ...
  \begin{tabular}{lrrrrrrr}
    ...
    leg & $4\times100$ & $4\times200$ & $4\times100$ & $4\times50$
        & twin $4\times100$ & straight & ceiling \\
    \midrule
    A vertex fetch  & $404$ [$286$--$538$]   & $234$ [$201$--$277$]
                    & $\mathbf{4.4}$ [$2$--$6$] & $4.1$ [$3$--$6$]
                    & $0.13$ & $10.9$ & $0.0013$ \\
    B cross-magnet  & $2600$ [$2106$--$3152$] & $1781$ [$1632$--$2154$]
                    & $\mathbf{1026}$ [$878$--$1209$] & $1136$ [$945$--$1627$]
                    & $512$ & $444{,}075$ & $46.4$ \\
    C plane-to-plane& $300$ [$252$--$335$]   & $213$ [$165$--$254$]
                    & $\mathbf{1.6}$ [$1$--$2$] & $1.2$ [$1$--$1$]
                    & $0.04$ & $6.6$ & $5.8\times10^{-7}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[tbp]
  ...
  it by a factor $6$ with the physics loss and $400$ with the twin, and a
  $4 \times 50$ residual network beats a $4 \times 200$ absolute-output network
  by a factor $130$. Source:
  ...
  \begin{tabular}{lrrrrrr}
    ...
    output design & $4\times50$ & $4\times100$ & $4\times200$
                  & $4\times50$ & $4\times100$ & $4\times200$ \\
    \midrule
    absolute states & $721$ & $397$ & $274$ & $637$ & $321$ & $237$ \\
    straight-line residual & $\mathbf{2.12}$ & $\mathbf{2.64}$ & ---
                           & $\mathbf{0.06}$ & $\mathbf{0.04}$ & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  absolute-output design sat thirty to fifty times above it. This is the
  hypothesis's own prediction, and it is confirmed with room to spare.
  ...
  \item \textbf{The data twin is extraordinary on the short legs}, $0.13\um$ on
  A and $0.04\um$ on C, that is $3400$ and $165$ times better than the
  absolute-output twin, and still twice as good as the physics loss on leg B,
  $512$ against $1026\um$. The gap between the two losses has widened in
  relative terms on the easy legs and narrowed on the hard one.
  ...
  B and $2.8 \times 10^{6}$ times on leg C. The absolute-output factors were
...
\begin{figure}[tbp]
  ...
  factor $46$ to $115$ in every band, so it is not an artefact of the momentum
...
\begin{table}[tbp]
  ...
  \begin{tabular}{crrrrrr}
    \toprule
    legs walked & absolute $4\times100$ & absolute $4\times100$ twin
                & residual $4\times100$ & residual $4\times100$ twin
                & residual $4\times50$ & particles \\
    \midrule
    $1$ & $246$      & $217$  & $\mathbf{1.2}$  & $\mathbf{0.03}$ & $1.1$  & $4563$ \\
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrrrrr}
    \toprule
    & absolute $4\times100$ & absolute $4\times100$ twin
    & residual $4\times100$ & residual $4\times100$ twin & residual $4\times50$ \\
    \midrule
...
\begin{itemize}
  \item \textbf{The label-free training is stable.} Across the campaign $232$
  runs were trained. All $24$ of the stage sweep, all $26$ of the general-leg
  first wave, $12$ of $13$ of the second wave, all $13$ of the reversed-polarity
  experiment and all $26$ of the residual redesign reached a confirmed stall,
  and $96$ of the $120$ in the architecture grid did. The $24$ that did not
  confirm are all in the architecture grid, all at width $32$ or depth $6$, and
  none of them diverged: they failed the confirmation pass, in which a fresh
  optimiser moves a small or deep network again rather than re-stalling. No run
  in the campaign blew up.
  \item \textbf{Where the numerical scheme is the limiting factor, the network
  sits on the scheme's own error}: $5202$ against $5198\um$ at $q = 2$ and
  $5371$ against $5401\um$ at $q = 4$, agreements of $0.1$ and $0.6$ percent.
  That is the technique doing exactly what it says it does.
  \item \textbf{Where the network is the limiting factor, the shortfall is
  identified.} It is optimisation, not capacity: the rank correlation between
  final training loss and test endpoint error over $96$ converged runs is
  $+0.965$, so every architecture and seed lies on one loss-error curve and the
  error is a readout of how far L-BFGS got.
  \item \textbf{At $4 \times 200$ the label-free loss beats its own supervised
  twin}, whose median is $12$ percent higher, $115$ against $128\um$, with a
  tighter seed spread.
  \item \textbf{On a field polarity for which no labels exist anywhere in the
  project}, the label-free network reaches the same floor, $229$ against
  $222\um$, and sits the same distance above that polarity's own ceiling, $10.4$
  against $9.8$ times.
\end{itemize}
...
\begin{enumerate}
  ...
  factor $11$ above the ceiling, while it reaches $0.13$ and $0.04\um$ on the
...
\begin{itemize}
  ...
  \item \textbf{One unconverged seed.} In the general-leg second wave,
  \texttt{w200\_physics\_s5} stopped at $111$ restarts, far short of its
  $400$-restart cap, in the "re-stalled but the medians moved" state described
  below. It was resubmitted but the farm queue was long, so it is reported
...
supervised twin sits a factor $11$ above it, and after two or three chained legs
that single leg dominates everything else. That is now the only failure left to
find, and Section~\ref{sec:outlook} says how to look for it.
...
Data set
\texttt{Data\_generation\_exploration/Official\_xdigi/training\_v2/train\_official\_v2.npz},
...
Field maps:
\texttt{/cvmfs/lhcb.cern.ch/lib/lhcb/DBASE/FieldMap/v8r1/cdf/field.v8r1.down.bin},
...
repository path given. Paths beginning \texttt{R/} are relative to
\texttt{R/}; the one path beginning \texttt{D/} is
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    \toprule
    figure & file in \texttt{Mini\_paper/figures/} & source in the repository \\
    \midrule
