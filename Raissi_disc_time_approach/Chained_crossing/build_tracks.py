...
`../Step_size_and_stage_grid` trains and scores on `results/grid_q<qq>.npz`,
whose evaluation splits are the whole test split of
`../Magnet_tracks_dataset`'s v3 set - 2,000 rows per stratum. The full-crossing
...
The hit is obtained the way `../MC_hit_comparison` obtains it: from
...
(`../Magnet_tracks_dataset/README.md`).
...
DENSE = os.path.join(ROOT, "Magnet_tracks_dataset", "results",
                     "dense_states.npz")
...
def test_rows(v3):
    """The v3 test rows, in the order `../Step_size_and_stage_grid` uses."""
