# C5_MC_hit_comparison — steps that end on a real simulated hit  (Block C, C5)

**What this is.** Every number in Block C so far has been an error against a
*computed* truth: the fine sixth-order reference of
[`../C1_Fine_reference`](../C1_Fine_reference), which propagates a state through the
...
| **the exact q = 8 scheme** | [`../C2_Exact_scheme_table`](../C2_Exact_scheme_table)'s root-finder, residual 1e-9 mm / mrad | the discretisation the scheme adds on top of the floor |
| **the grid networks** | the trained checkpoints of [`../C3_Step_size_and_stage_grid`](../C3_Step_size_and_stage_grid) at q = 8, both arms, four architectures | what the network adds on top of both |
...
map, and the sample is a MagUp sample: `../C0_Magnet_tracks_dataset`'s C0
...
`../C0_Magnet_tracks_dataset`'s split is 60/20/20 by particle over a 6,000-particle
...
* **`vs_reference`** — against the fine reference's own answer from the same
  ...
  `../C3_Step_size_and_stage_grid` measures, and it is what says whether a
...
(`../C1_Fine_reference` C1.4) — the material term is **36,000 times larger** on the
