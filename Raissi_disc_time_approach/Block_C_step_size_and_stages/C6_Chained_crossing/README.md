# C6_Chained_crossing — every trained network walked across the whole magnet  (Block C, C6)

**What this is.** [`../C3_Step_size_and_stage_grid`](../C3_Step_size_and_stage_grid)
...
[`../C3_Step_size_and_stage_grid`](../C3_Step_size_and_stage_grid) are opened
read-only.
...
[`../C0_Magnet_tracks_dataset`](../C0_Magnet_tracks_dataset). For a track with N steps
...
[`../C3_Step_size_and_stage_grid`](../C3_Step_size_and_stage_grid) itself uses.
...
| **the fine reference** at the far plane | the v3 set's own `Y`: an RK6 march at 0.1 mm on the MagUp map. Its own accuracy over a crossing is 5e-5 µm ([`../C1_Fine_reference`](../C1_Fine_reference) C1.4) |
| **the particle's real hit** at the far plane | forward → its first SciFi crossing, backward → its last UT crossing, read straight out of the harvested MCHit states the way [`../C5_MC_hit_comparison`](../C5_MC_hit_comparison) reads them |
...
([`../C0_Magnet_tracks_dataset`](../C0_Magnet_tracks_dataset)), so it is the right
...
* **single step (grid C4)** — the same architecture's own single-step
  ...
  `../C3_Step_size_and_stage_grid/results/table_cells.csv`. It is measured on the
...
* **material floor** — the particle's real far-plane hit against the fine
  ...
  of any kind can predict ([`../C5_MC_hit_comparison`](../C5_MC_hit_comparison)).
...
`../C3_Step_size_and_stage_grid/results/<tag>.json`. Over the 720 records and the
...
consistent with [`../C5_MC_hit_comparison`](../C5_MC_hit_comparison)'s 1,815 µm on its
