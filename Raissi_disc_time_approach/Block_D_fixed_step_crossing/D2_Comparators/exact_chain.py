...
def run_one(N, q, D, out_dir, split="test", n_cap=None):
    ...
    t0 = time.time()
    cur = S.copy()
    per_plane_med, per_plane_p95, n_fail, n_eval = [0.0], [0.0], 0, 0
    for k in range(N):
        zk = Z0 + k * dz
        ...
        for i in range(n):
            ...
            n_eval += used
        cur = nxt
        err = np.abs(cur[:, :2] - truth[:, (k + 1) * stride, :2]).max(axis=1) * 1e3
        per_plane_med.append(float(np.median(err)))
        per_plane_p95.append(float(np.quantile(err, 0.95)))
    ...
    rec = {"N": N, "q": q, "dz_mm": dz, "split": split, "n": int(n),
           ...
           "per_plane_pos_med_um": per_plane_med,
           "per_plane_pos_p95_um": per_plane_p95,
...
def main(argv=None):
    ...
    if a.all:
        for N in N_VALUES:
            for q in QS:
                if not os.path.exists(os.path.join(a.out, "exact_N%03d_q%02d.json" % (N, q))):
