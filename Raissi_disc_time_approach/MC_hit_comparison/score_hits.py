...
2. **the exact q = 8 Gauss-Legendre scheme** - `../Exact_scheme_table`'s
...
3. **the grid networks** - the trained checkpoints of
   `../Step_size_and_stage_grid` at q = 8, both the physics arm and the data
...
* `vs_reference` - against the fine reference's own answer from the same start
  state. This is the field-only error, the quantity `../Step_size_and_stage_grid`
...
GRID = os.path.abspath(os.path.join(HERE, "..", "Step_size_and_stage_grid"))
SCHEME_DIR = os.path.abspath(os.path.join(HERE, "..", "Exact_scheme_table"))
...
# crossing (`../Step_size_and_stage_grid/results/reading_best_architecture.csv`),
...
def main(argv=None):
    ...
    meta = {
        ...
        "scheme": {"q": SCHEME_Q, "solver": "Exact_scheme_table/exact_solver.py",
