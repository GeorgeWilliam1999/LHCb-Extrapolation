...
(`../Magnet_tracks_dataset/README.md`), so `Y` is the right start state pushed
...
| non-electron | as `../Magnet_tracks_dataset` C0.1: an electron's "material effect" is bremsstrahlung, a different measurement |
...
| the particle is **not** in the v3 train or val split | `../Magnet_tracks_dataset`'s split is 60/20/20 by particle over a 6,000-particle subsample of the same population, and its seed differs from v2's, so a v2-test particle can be a v3-train one. Those are removed. |
