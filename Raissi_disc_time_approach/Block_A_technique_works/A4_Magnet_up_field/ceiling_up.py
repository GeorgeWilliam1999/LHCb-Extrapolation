...
The solver is `../../Block_0_first_pass/S1_Simple_first_pass/exact_scheme.py` (Task-1 result: leg B,
q = 8, 29 um median on MagDown), copied here in its minimal form because that
file reaches into `S0_Baseline_data_exploration/reference_card.py`, whose `deriv`
...
numbers:
  ...
  stratified  32 leg-B legs drawn stratified in momentum, on their own start
              planes - the population `../../Block_0_first_pass/S1_Simple_first_pass` used, which
  ...
  test-states the frozen leg's own test split, the exact states the networks
              ...
              network floor. `../A1_Stage_count_sweep/measure_scheme_ceiling.py`
...
def solve_leg(S0, z0, z1, tab, field, maxfev=20000):
    """One leg's implicit stage equations. Verbatim from S1_Simple_first_pass,
...
def stratified_rows(d, leg, n, rng):
    """Momentum-stratified pick, verbatim from S1_Simple_first_pass."""
    ...
    with open(os.path.join(RES, "ceiling_summary.json"), "w") as f:
        json.dump({"q": Q, "leg": "B", "n_states": N_STATES, "seed": SEED,
                   ...
                   "note": "per_field = 32 momentum-stratified legs "
                           "(the S1_Simple_first_pass population); "
