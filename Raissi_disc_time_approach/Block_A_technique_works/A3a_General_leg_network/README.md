# A3a_General_leg_network — one network for legs A, B and C  (A3a)

**Question.** The verified baseline (`../../Block_0_first_pass/S2b_One_step_network_v2`) trains a network on
...
`../A3b_Chained_legs/chain.py` read the architectures out of the result files, so
...
`../../Block_0_first_pass/S1_Simple_first_pass/results/scheme_error_vs_q.csv` measured the exact scheme on
...
2. **Generalising the leg is expensive.** The same physics loss, same q, same
   ...
   (`../../Block_0_first_pass/S2b_One_step_network_v2`). Asking one network to carry (z0, dz) as inputs and
