...
Why this script exists. `../../Block_0_first_pass/S1_Simple_first_pass/results/scheme_error_vs_q.csv`
...
`../../Block_0_first_pass/S1_Simple_first_pass/exact_scheme.py`, so the ceiling measured here and the
...
FIRST_PASS = os.path.join(HERE, "..", "..", "Block_0_first_pass", "S1_Simple_first_pass")

# exact_scheme.py resolves its own imports off sys.path; give it what it needs
# and import the solver rather than copying it.
sys.path.insert(0, os.path.join(FIRST_PASS, "..", "S0_Baseline_data_exploration"))
...
def measure(q, n, split, seed=20260718):
    ...
    # with, and the one `../../Block_0_first_pass/S1_Simple_first_pass` aggregated its own scan with:
    ...
    out = {
        ...
        "solver": "solve_leg imported from ../../Block_0_first_pass/S1_Simple_first_pass/exact_scheme.py",
