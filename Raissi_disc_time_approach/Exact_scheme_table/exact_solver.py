...
This is `../Simple_first_pass/exact_scheme.py`'s `solve_leg` generalised in
three ways and in no others:
  ...
  * **the field is an argument.** That file calls `deriv(S, znodes)` with no
    ...
    (`../Magnet_tracks_dataset/README.md`), so the polarity has to be chosen
...
all get the same code (as `../Stage_count_sweep` imports the first pass's).
"""
