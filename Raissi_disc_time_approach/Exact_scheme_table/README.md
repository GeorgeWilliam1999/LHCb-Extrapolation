# Exact_scheme_table — the scheme's own error, no network  (Block C, C2)
...
[`../Magnet_tracks_dataset`](../Magnet_tracks_dataset), for ten stage counts and
...
length. `../Simple_first_pass` and `../Stage_count_sweep` measured that ceiling
...
the fine sixth-order reference of [`../Fine_reference`](../Fine_reference)
...
[`../Simple_first_pass/exact_scheme.py`](../Simple_first_pass/exact_scheme.py)'s
...
* **the field is an argument.** That file calls the ODE with no field, i.e. the
  ...
  ([`../Magnet_tracks_dataset/README.md`](../Magnet_tracks_dataset/README.md)),
...
magnet?" was the feasibility question `../Simple_first_pass` opened and this
...
* **The reference floor is length-dependent, and the quoted 5e-5 µm is the
  full-crossing figure.** `../Fine_reference` established 5e-5 µm as the worst
...
This is the same verdict `../Fine_reference` reached from the other side and it
...
in `../One_step_network_v2`, `../General_leg_network` and `../Chained_legs` was
...
`../Simple_first_pass` reported for q = 8 on leg B; the difference is population,
...
comparison `../General_leg_network`'s verdict was really about.
...
(`../Fine_reference` C1.3) — a factor 7 in the scheme's favour, at four orders
