# Magnet_tracks_dataset — cross-magnet steps of every length  (Block C, C0)
...
by the fine sixth-order reference of [`../Fine_reference`](../Fine_reference),
...
on a mixture dominated by one. `../General_leg_network`'s verdict was that a
...
on 200,000 random in-map points, re-confirming `../Magnet_up_field`'s A4.1 — so
...
2. Experiments that only ever compared a network with that same engine
   (`../One_step_network_v2`, `../General_leg_network`, `../Chained_legs`, the
...
3. `../Magnet_up_field` is framed as training "on a field polarity for which no
...
`../_shared/prepare.magnet_leg_rows` — the same function `../Fine_reference`
...
per leg, matching `../Fine_reference`'s C1.3 figure of 0.12–0.13 s per track at
