# Magnet_up_field — training the one-step network where no labels exist
...
The down-field build reproduces `../One_step_network_v2/results/frozen_leg_data.npz`
...
* The headline MagDown row is `../Network_size_and_seed_study` at width 50,
  depth 4 — the same architecture, the same ten-seed protocol and the same
  single-thread arithmetic as these runs. `../One_step_network_v2`'s three-seed
...
* The ceiling is quoted two ways because the two populations give different
  ...
  µm, reproduces `../Stage_count_sweep/results/scheme_ceiling_same_population_q08.json`
  ...
  row is the `../Simple_first_pass` population, 32 leg-B legs drawn stratified
...
* `../Simple_first_pass/exact_scheme.py` cannot be imported for an up-field
  measurement: its `deriv` comes from `Baseline_data_exploration/reference_card.py`,
